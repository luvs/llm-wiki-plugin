...
The three layers are **raw sources** (the user's curated source material — articles, papers, PDFs, transcripts; effectively immutable, the LLM reads but never modifies their content — the sole exception is the `processed:` frontmatter flag on raw markdown files, which the ingest workflow flips to `true` once a source is compiled), **the wiki** (a directory of LLM-generated markdown pages — entity pages, concept pages, comparisons, summaries; the LLM owns this layer entirely), and **the schema** (a `SCHEMA.md` file at the wiki root that documents the conventions for this particular wiki — page types, naming rules, tag taxonomy, ingest workflow customizations; co-evolved with the user).
...
The full workflow is in `references/ingest-workflow.md`; what follows is the shape of it. When the user doesn't name a specific source ("what's left to ingest?", a batch pass, `/wiki:ingest` with no argument), first discover the queue with `python scripts/wiki_ingest_queue.py .` and present the pending sources for the user to choose from — raw markdown files carry a `processed:` flag (`false` or absent means pending), while PDFs and other non-markdown sources are pending until a `wiki/sources/` page references them via `raw:`. When a new source arrives, first write the source itself into `raw/` (verbatim; if it's a web article, use the markdown form, with `processed: false` in its frontmatter). Then read the source — chunked if large — and write a single source-summary page in `wiki/sources/`, named after the source slug, with full frontmatter and citations back to the raw file. Then identify which existing entity and concept pages this source touches; for each, surgically update the relevant section using `str_replace` rather than rewriting. Identify any new entities or concepts the source introduces and create new pages for them, linking from related existing pages so they don't become orphans. Update `index.md` (or the relevant shard) with the new pages. Append a single line to `log.md` with the date, operation type, and source title. Discuss the takeaways with the user as a final step — what surprised them, what's worth following up on — and offer to file that discussion back as a synthesis page.
...
- `scripts/wiki_ingest_queue.py` — list raw sources still pending ingest (markdown by `processed:` flag, PDFs/other by whether a `wiki/sources/` page references them); powers the "what's left to ingest?" selection
