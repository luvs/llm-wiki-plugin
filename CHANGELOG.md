...
- **Ingest queue — `/wiki:ingest` with no argument now asks what to process.** Raw sources are tracked as pending vs done: markdown raw files carry a `processed:` frontmatter flag (`false`, or no key at all, means pending; the ingest workflow flips it to `true` as its last step), and PDFs/other non-markdown sources count as pending until a `wiki/sources/` page references them via `raw:`. The new `scripts/wiki_ingest_queue.py` applies both rules (no third-party deps, never reads binary contents) and prints the pending list as text or `--json`. The ingest command and `references/ingest-workflow.md` (new Step 0.5 discovery + Step 9b mark-processed) drive the interactive selection. `SCHEMA.md.template` documents the convention and `init_wiki.py --upgrade` flags it for existing wikis (`[0.4.0]`).
...
- **Raw sources are no longer strictly immutable.** The skill previously stated the LLM never modifies raw sources; the ingest queue introduces one narrow exception — flipping the `processed:` flag in a raw markdown file's frontmatter once it's compiled. Raw source *content* remains untouched. SKILL.md and the ingest workflow's anti-patterns document the boundary.
